...
execute if score $sec math < $10 math run tellraw @a [ \
    {text:"| ",color:"gray",bold:true}, \
    {translate:"mcm.game.stats.time",color:"gold",bold:false,with:[ \
        {selector:"@s",color:"green"}, \
        {translate:"mcm.time.minsec",color:"gold",with:[ \
            {score:{objective:"math",name:"$min"},color:"gold"}, \
            {translate:"mcm.time.leading.0",color:"gold",with:[ \
                {score:{objective:"math",name:"$sec"},color:"gold"}\
            ]} \
        ]} \
    ]} \
]
