...
execute if score $library_flip CmdData matches 1 if predicate mcm:5050chance as @e[type=marker,tag=sound_marker,limit=1,sort=random] at @s run playsound minecraft:ambient.basalt_deltas.mood block @a ~ ~ ~ 1 1 
