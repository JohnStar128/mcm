...
$execute if score slot temp matches 0.. run tag @s[nbt=!{Inventory:[{Slot:$(slot)b}]}] add item_replace
...
$execute if entity @s[tag=item_replace] if score slot temp matches 0.. run item replace entity @s hotbar.$(slot) with $(id)$(nbt)
