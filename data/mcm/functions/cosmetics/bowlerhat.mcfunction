execute if entity @s[advancements={mcm:secrets/vineyard/vineyard=true}] unless entity @s[nbt={Inventory:[{Slot:103b,id:"minecraft:diamond_hoe",Count:1b,tag:{CustomModelData:1113}}]}] at @s run playsound minecraft:item.armor.equip_leather block @s ~ ~ ~ 1 0
...
execute if entity @s[advancements={mcm:secrets/vineyard/vineyard=false}] run tellraw @s ["", {"text":"You can't equip this hat yet","color":"red"}]
