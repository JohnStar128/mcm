execute if entity @s[advancements={mcm:secrets/launchpad/spaceship=true}] unless entity @s[nbt={Inventory:[{Slot:103b,id:"minecraft:diamond_hoe",Count:1b,tag:{CustomModelData:1111}}]}] at @s run playsound minecraft:item.armor.equip_iron block @s ~ ~ ~ 1 0
...
execute if entity @s[advancements={mcm:secrets/launchpad/spaceship=false}] run tellraw @s ["", {"text":"You can't equip this hat yet","color":"red"}]
