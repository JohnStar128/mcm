...
execute at @e[type=marker,tag=active_target_spawn] run summon zombie ~ ~ ~ {Tags:["test_range_entity","test_range_target"],NoAI:1b,Silent:1b,PersistenceRequired:1b}
effect give @e[type=zombie,tag=test_range_target] invisibility infinite 100 true
effect give @e[type=zombie,tag=test_range_target] resistance infinite 100 true
...
#> Target was hit
execute if entity @a[advancements={mcm:hit_detection/test_range_hit=true}] at @e[type=zombie,tag=test_range_target] run summon firework_rocket ~ ~1 ~ {FireworksItem:{id:firework_rocket,Count:1,tag:{Fireworks:{Flight:0,Explosions:[{Colors:[I;4312372],FadeColors:[I;14602026]}]}}}}
execute if entity @a[advancements={mcm:hit_detection/test_range_hit=true}] run kill @e[type=armor_stand,tag=test_range_target]
execute if entity @a[advancements={mcm:hit_detection/test_range_hit=true}] run kill @e[type=zombie,tag=test_range_target]
...
# teleporter
execute as @a[predicate=mcm:bounding_boxes/test_range,scores={carrot=1..},nbt={SelectedItem:{tag:{CustomModelData:1112}}}] at @s run function mcm:lobby/test_range/tp_target
# adrenaline
execute as @a[predicate=mcm:bounding_boxes/test_range,scores={carrot=1..},nbt={SelectedItem:{tag:{CustomModelData:1113}}}] at @s run effect give @s speed 5 3 true
execute as @a[predicate=mcm:bounding_boxes/test_range,scores={carrot=1..},nbt={SelectedItem:{tag:{CustomModelData:1113}}}] at @s run effect give @s jump_boost 5 2 true
execute as @a[predicate=mcm:bounding_boxes/test_range,scores={carrot=1..},nbt={SelectedItem:{tag:{CustomModelData:1113}}}] at @s run playsound minecraft:entity.generic.drink block @a ~ ~ ~ 1 1 0
