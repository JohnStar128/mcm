...
execute if score $scrapclock CmdData matches 20.. if predicate mcm:scraprng run function mcm:game/loops/spawnscrap
...
execute as @a[tag=murderer,nbt={SelectedItem:{id:"minecraft:compass",Count:1b,tag:{CustomModelData:1111}}}] at @a[tag=innocent,tag=!spectating,limit=1,sort=nearest] run function mcm:game/dist
execute as @a[tag=murderer,nbt={SelectedItem:{id:"minecraft:compass",Count:1b,tag:{CustomModelData:1111}}}] run title @s actionbar ["Closest innocent is ", {"score": {"name": "$out", "objective": "temp"},"color":"gold"}, " blocks away"]
...
execute as @a[tag=murderer,tag=!retrieved,scores={throwKnife=1..}] if score @s canGetRetrieval matches 1.. if entity @e[tag=knifeCosmetic] run item replace entity @s hotbar.1 with minecraft:carrot_on_a_stick{NoDrop:1b,CustomModelData:1111}
