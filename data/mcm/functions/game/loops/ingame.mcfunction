...
#> Win conditions
# Murderer victory
execute if entity @a[tag=innocent,scores={dead=1..}] run tellraw @a ["", "\n", {"text":"The Murderer has won!","color":"red"}, "\n"]
execute if entity @a[tag=innocent,scores={dead=1..}] run scoreboard players set $gamestate CmdData 2

# Innocents victory
execute if entity @a[tag=murderer,scores={dead=1..}] run tellraw @a ["", "\n", {"text":"The Innocents have won!","color":"green"}, "\n"]
execute if entity @a[tag=murderer,scores={dead=1..}] run scoreboard players set $gamestate CmdData 2