...
execute as @e[type=marker,tag=ScrapSpawn,limit=1,sort=random] at @s if score @e[type=marker,tag=ScrapRNG,limit=1] scrapRNG matches 1.. run summon item ~ ~ ~ {Item: {id: "minecraft:netherite_scrap", Count: 1b, tag: {no_drop_on_death:1b, CustomModelData: 1, Tags: ["KeyItem"], display: {Lore: ['[{"translate":"mcm.item.scrap.lore","italic":false}]'], Name: '{"translate":"mcm.item.scrap","italic":false}'}}}} 
