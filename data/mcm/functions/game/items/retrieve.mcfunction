...
$execute if items entity @s weapon.mainhand carrot_on_a_stick[custom_data~{knife_retrieve:1}] run return run function mcm:game/items/knife_retrieve {retrieve: $(retrieve), cost: $(cost)}



execute store result score scrap temp run clear @s netherite_scrap 0

data modify storage mcm:args nbt.loadout set from entity @s SelectedItem.components.minecraft:custom_data.retrieve_loadout
